...
\chapter*{}
\begin{center}
{\textbf{\huge Package `autoSDM'}}
\par\bigskip{\large \today}
\end{center}
\inputencoding{utf8}
\ifthenelse{\boolean{Rd@use@hyper}}{\hypersetup{pdftitle = {autoSDM: Species Distribution Modeling using Alpha Earth Embeddings}}}{}
\begin{description}
\raggedright{}
...
magrittr}
\item[URL]\AsIs{}\url{https://github.com/James-Longo/autoSDM}\AsIs{}
\item[BugReports]\AsIs{}\url{https://github.com/James-Longo/autoSDM/issues}\AsIs{}
...
\inputencoding{utf8}
\HeaderA{.run\_command\_with\_timer}{Shared internal helper for live-updating timer}{.run.Rul.command.Rul.with.Rul.timer}
\keyword{internal}{.run\_command\_with\_timer}
%
\begin{Description}
...
\begin{Arguments}
\begin{ldescription}
\item[\code{label}] Text label for the timer.

\item[\code{active}] Boolean logic for activation.
\end{ldescription}
\end{Arguments}
\inputencoding{utf8}
...
\end{Usage}
\inputencoding{utf8}
\HeaderA{ensure\_autoSDM\_dependencies}{Ensure dependencies are met (Internal)}{ensure.Rul.autoSDM.Rul.dependencies}
\keyword{internal}{ensure\_autoSDM\_dependencies}
%
\begin{Description}
...
\item[\code{predict\_coords}] Data frame of coordinates to evaluate at. Must have (longitude, latitude, year, present).

\item[\code{methods}] Character vector of method names.

\item[\code{scale}] Resolution in meters for sampling. Defaults to 10.
...
\item[\code{min\_leaf\_population}] Min leaf size.

\item[\code{bag\_fraction}] Bagging fraction.

\item[\code{shrinkage}] Learning rate.

\item[\code{max\_nodes}] Max nodes.

\item[\code{variables\_per\_split}] Vars per split.

\item[\code{lambda\_}] Penalty.

\item[\code{polynomial}] Degree.

\item[\code{output\_dir}] Directory to save results.

\item[\code{gee\_project}] Optional Project ID.

\item[\code{python\_path}] Optional Python path.
...
\end{Value}
\inputencoding{utf8}
\HeaderA{format\_data}{Format Data for autoSDM}{format.Rul.data}
%
\begin{Description}
...
%
\begin{Value}
A standardized data frame ready for `autoSDM()` with lowercase column names.
\end{Value}
\inputencoding{utf8}
...
\end{Arguments}
\inputencoding{utf8}
\HeaderA{install\_autoSDM}{Install autoSDM Python Dependencies}{install.Rul.autoSDM}
%
\begin{Description}
...
\begin{Usage}
\begin{verbatim}
install_autoSDM(method = "virtualenv", envname = "r-autoSDM")
\end{verbatim}
\end{Usage}
%
\begin{Arguments}
...
\item[\code{models\_list}] List of model results from autoSDM
\end{ldescription}
\end{Arguments}
\inputencoding{utf8}
...
\end{Arguments}
\inputencoding{utf8}
\HeaderA{train\_autoSDM\_internal}{Internal Training Pipeline}{train.Rul.autoSDM.Rul.internal}
\keyword{internal}{train\_autoSDM\_internal}
%
\begin{Description}
